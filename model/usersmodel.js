...
var users = {
	...
	getUserById: function(forms, id, cb){
		console.log('03model', forms, id);
		var sql = "select * from "+forms.tablename+" where USERID = "+id.id;
	...
	updateUsers: function(forms, inputData,id, cb){
// console.log('inputData', inputData);
		...
		var whereQuery = '';
			...
			// whereQuery = .substring(0, whereQuery.length -3);
		var sql = "update "+forms.tablename+" set "+setQuery+", UPDATIONDATE=now() WHERE USERID="+id.id;
	...
	deleteUsers: function(forms, id, cb){
		var sql = "delete from "+forms.tablename+" where userid="+id.id;
		pool.getConnection(function(error, connection){
			if (error) {
				console.log('connection error'+error);
			}
