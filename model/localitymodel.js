...
var locality = {
	...
	listLocalityByCity: function(id, cb){
		var sql = "SELECT * FROM locality l INNER JOIN city c ON l.CITYID = c.CITYID where c.CITYID="+id.id;
		pool.getConnection(function(error, connection){
			if (error) {
				console.log('connection error'+error);
			}
	...
	localityUpdate: function(inputData,id, cb){
		console.log('01', inputData, '02', id);
		var sql = "update locality set LOCALITYNAME='"+inputData.lname+"', CITYID="+inputData.cid+", UPDATIONDATE= now() where LOCALITYID="+id.localityid;
		pool.getConnection(function(error, connection){
			if (error) {
				console.log('connection error'+error);
			}
